...
def _base_url():
    ...
    if configured:
        return configured
    # Same X-Forwarded-Proto fallback as verify_api_csrf_origin() in
    # app/__init__.py — cloudflared doesn't set the header, so assume https
    # when we know we're behind an HTTPS tunnel rather than trusting the
    # plain-HTTP scheme of the tunnel-to-Waitress hop.
    scheme = request.headers.get('X-Forwarded-Proto') or (
        'https' if app_pkg._https_tunnel_configured() else request.scheme
    )
