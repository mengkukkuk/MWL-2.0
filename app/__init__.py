...
def _https_tunnel_configured():
    """True when the app is deployed behind an HTTPS-terminating tunnel
    (ngrok, tailscale serve, or a Cloudflare Tunnel) that talks plain HTTP to
    this process. Used both for the SESSION_COOKIE_SECURE default and to fill
    in X-Forwarded-Proto when the tunnel doesn't set it itself — notably,
    cloudflared does not set X-Forwarded-Proto by default, and Quick Tunnels
    (no domain, no ingress config) have no way to add it."""
    return bool(
        os.getenv('NGROK_DOMAIN', '').strip()
        or os.getenv('TAILSCALE_DOMAIN', '').strip()
        or os.getenv('CLOUDFLARE_TUNNEL', '').strip()
    )


# Cookie 'Secure' flag — explicit env wins; otherwise auto: ON when any HTTPS-fronted
# deployment is configured, OFF for plain-HTTP LAN / raw Tailscale device IPs (so the
# session cookie isn't silently dropped by the browser).
...
if _secure_env in ('true', '1', 'yes'):
    ...
    app.config['SESSION_COOKIE_SECURE'] = _https_tunnel_configured()
...
def verify_api_csrf_origin():
    ...
    # cloudflared (named tunnels and Quick Tunnels alike) does not set
    # X-Forwarded-Proto by default, and Quick Tunnels have no ingress config
    # to add it — so falling back to request.scheme (the plain-HTTP hop from
    # the tunnel to Waitress) would make proxy_host_url always "http://...",
    # never matching the browser's real "https://..." Origin. When we know
    # we're behind an HTTPS tunnel, assume https instead of trusting scheme.
    forwarded_proto = request.headers.get('X-Forwarded-Proto') or (
        'https' if _https_tunnel_configured() else request.scheme
    )
