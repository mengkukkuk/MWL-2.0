...
export function AllowancePage() {
  ...
  const allowanceQuery = useQuery({ queryKey: ['allowance', user?.member_id, selectedMonth], queryFn: () => allowanceApi.list(user!.member_id, year, month), enabled: !!user });
  const months = recentMonths(24);

  const saveMutation = useMutation({
    mutationFn: () => editing ? allowanceApi.update(editing.ID, form) : allowanceApi.create(form),
    onSuccess: () => { notifySuccess(editing ? 'Allowance updated' : 'Allowance added'); close(); void queryClient.invalidateQueries({ queryKey: ['allowance', user?.member_id, selectedMonth] }); },
    onError: (error) => notifyError(error, 'Unable to save this allowance.'),
  });
  const deleteMutation = useMutation({
    mutationFn: (id: number) => allowanceApi.remove(id),
    onSuccess: () => { notifySuccess('Allowance removed'); void queryClient.invalidateQueries({ queryKey: ['allowance', user?.member_id, selectedMonth] }); },
    onError: (error) => notifyError(error, 'Unable to remove this allowance.'),
  });

  const startCreate = () => { setEditing(null); setForm(blankForm(user?.member_id ?? '')); open(); };
  ...
  return (
    <Stack gap="xl">
      <PageHeader eyebrow="Special days" title="Allowance" description="Record allowance-eligible days with the same clear audit trail as your worklog." breadcrumb="Allowance" actions={<Button leftSection={<IconPlus size={17} />} onClick={startCreate}>Add allowance</Button>} />
